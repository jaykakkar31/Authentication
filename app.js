...
var GoogleStrategy = require("passport-google-oauth20").Strategy; //5
...
// app.get("https://accounts.google.com/logout", function (req, res) {
  ...
  User.findById(req.user.id, function (err, foundUser) {
    if (err) {
      ...
      if (foundUser) {
        ...
        foundUser.save();
        res.render("secrets",{mySecret:foundUser.secrets})
  ...
  if (req.isAuthenticated()) {
    ...
    User.findById(req.user.id, function (err, foundUser) {
      if (err) {
        ...
        res.render("secrets", { mySecret: foundUser.secret });
      }
    });
  } else {
    res.redirect("/login");
